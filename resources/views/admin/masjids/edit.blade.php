...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.masjid.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.masjids.update", [$masjid->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('negeri') ? 'is-invalid' : '' }}" name="negeri_id" id="negeri_id">
                    @foreach($negeris as $id => $negeri)
                        <option value="{{ $id }}" {{ ($masjid->negeri ? $masjid->negeri->id : old('negeri_id')) == $id ? 'selected' : '' }}>{{ $negeri }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $masjid->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $masjid->location) }}">
