...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.profilAhli.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.profil-ahlis.update", [$profilAhli->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_penuh">{{ trans('cruds.profilAhli.fields.nama_penuh') }}</label>
                <input class="form-control {{ $errors->has('nama_penuh') ? 'is-invalid' : '' }}" type="text" name="nama_penuh" id="nama_penuh" value="{{ old('nama_penuh', $profilAhli->nama_penuh) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_kad_pengenalan') ? 'is-invalid' : '' }}" type="text" name="no_kad_pengenalan" id="no_kad_pengenalan" value="{{ old('no_kad_pengenalan', $profilAhli->no_kad_pengenalan) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ictype') ? 'is-invalid' : '' }}" name="ictype_id" id="ictype_id">
                    @foreach($ictypes as $id => $ictype)
                        <option value="{{ $id }}" {{ ($profilAhli->ictype ? $profilAhli->ictype->id : old('ictype_id')) == $id ? 'selected' : '' }}>{{ $ictype }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jantina') ? 'is-invalid' : '' }}" name="jantina_id" id="jantina_id">
                    @foreach($jantinas as $id => $jantina)
                        <option value="{{ $id }}" {{ ($profilAhli->jantina ? $profilAhli->jantina->id : old('jantina_id')) == $id ? 'selected' : '' }}>{{ $jantina }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="team_id">{{ trans('cruds.profilAhli.fields.team') }}</label>
                <select class="form-control select2 {{ $errors->has('team') ? 'is-invalid' : '' }}" name="team_id" id="team_id">
                    @foreach($teams as $id => $team)
                        <option value="{{ $id }}" {{ ($profilAhli->team ? $profilAhli->team->id : old('team_id')) == $id ? 'selected' : '' }}>{{ $team }}</option>
                    @endforeach
                </select>
                @if($errors->has('team_id'))
                    <span class="text-danger">{{ $errors->first('team_id') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.profilAhli.fields.team_helper') }}</span>
            </div>
            <div class="form-group">
